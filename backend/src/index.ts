...
async function sendStatusToClient(ws: WebSocket) {
  try {
    ...
    const statuses: ServiceStatus[] = services.map((service) => {
      ...
      const lastUpdated = quotas.reduce<Date>((max, q) => (q.updatedAt > max ? q.updatedAt : max), new Date(0));
...
async function refreshQuotas() {
  ...
  try {
    ...
    const staggerDelayMs = services.length > 1
      ? (intervalMinutes * 60 * 1000) / services.length
      : 0;
          ...
          try {
            ...
            for (const quota of status.quotas) {
              await db.run(
                ...
                [require('uuid').v4(), quota.serviceId, quota.metric, quota.used, new Date().toISOString()]
              );
            }
          } catch (dbError) {
