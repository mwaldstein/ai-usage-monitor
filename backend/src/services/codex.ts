  ...
  async fetchQuotas(): Promise<UsageQuota[]> {
    ...
    try {
      ...
      if (!this.service.bearerToken && !this.service.apiKey) {
        console.error(`[Codex:${serviceName}] ERROR: No authentication provided. Please provide either a Bearer token or session cookie.`);
        return quotas;
      }
      ...
      if (this.service.bearerToken) {
        console.log(`[Codex:${serviceName}] Using Bearer token auth (${this.service.bearerToken.length} chars)`);
      }
      
      if (this.service.apiKey) {
        console.log(`[Codex:${serviceName}] Using Cookie auth (${this.service.apiKey.length} chars)`);
      }
      ...
      if (this.service.apiKey) {
        ...
        if (accountId) {
          ...
          console.log(`[Codex:${serviceName}] WARNING: No account identifier found in cookie (_account/account_id)`);
        }
      }
      ...
      console.log(`[Codex:${serviceName}] Request auth: bearer=${!!this.service.bearerToken} cookie=${!!this.service.apiKey}`);
      ...
      console.log(`[Codex:${serviceName}] Response data keys: ${Object.keys(response.data).join(', ')}`);
      ...
      if (data.rate_limit?.primary_window) {
        ...
        console.log(`[Codex:${serviceName}] ✓ Added rolling_5hour quota: ${burnDownPercent.toFixed(1)}% remaining`);
      } else {
      ...
      if (data.rate_limit?.secondary_window) {
        ...
        console.log(`[Codex:${serviceName}] ✓ Added weekly quota: ${burnDownPercent.toFixed(1)}% remaining`);
      } else {
      ...
      if (data.code_review_rate_limit?.primary_window) {
        ...
        console.log(`[Codex:${serviceName}] ✓ Added code_reviews quota: ${burnDownPercent.toFixed(1)}% remaining`);
      } else {
