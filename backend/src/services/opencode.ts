...
export class OpenCodeService extends BaseAIService {
  ...
  private parseHydrationData(html: string): { 
    ...
    try {
        ...
        for (const call of allCalls) {
          ...
          if (dataIndexMatch) {
            ...
            if (startMatch) {
              ...
              if (dataStr.includes('rollingUsage') || dataStr.includes('weeklyUsage') || dataStr.includes('plan')) {
                const parsed = this.parseObject(dataStr);
      ...
      const subWithUsage = html.match(/\$R\[\d+\]=\{[^}]*plan:[^}]*rollingUsage:\$R\[\d+\]=\{[^}]*\}[^}]*weeklyUsage:\$R\[\d+\]=\{[^}]*\}[^}]*\}/);
      if (subWithUsage) {
        // Extract rollingUsage: $R[X]={status:"...",resetInSec:Y,usagePercent:Z}
        const rollingMatch = subWithUsage[0].match(/rollingUsage:\$R\[\d+\]=(\{status:"[^"]+",resetInSec:\d+,usagePercent:\d+\})/);
        ...
        const weeklyMatch = subWithUsage[0].match(/weeklyUsage:\$R\[\d+\]=(\{status:"[^"]+",resetInSec:\d+,usagePercent:\d+\})/);
      ...
      if (!result.rollingUsage || !result.weeklyUsage) {
        const allUsageMatches = html.matchAll(/(rollingUsage|weeklyUsage):\$R\[\d+\]=(\{status:"[^"]+",resetInSec:\d+,usagePercent:\d+\})/g);
  ...
  async fetchQuotas(): Promise<UsageQuota[]> {
    try {
      ...
      if (!workspaceId) {
        console.warn('No workspace ID found for opencode service. Set baseUrl to include workspace ID (e.g., https://opencode.ai/workspace/WRK_ID)');
        return [];
      }

      // Check if API key (session cookie) is provided
      if (!this.service.apiKey) {
        console.warn('No session cookie provided for opencode service. Authentication required. Please copy your session cookie from the browser and paste it as the API key.');
        return [];
      }
      ...
      if (data.billing || data.subscription || data.rollingUsage || data.weeklyUsage) {
        ...
        const isAuthPage = html.includes('<title>OpenAuth</title>') || 
        ...
        if (isAuthPage) {
          ...
          const error = new Error('Authentication failed: Session cookie expired or invalid. Please get a new session cookie from your browser.');
      ...
      if (rollingUsage) {
        ...
        console.log(`Added rolling usage quota: ${pagePercent}% page value → ${burnDownPercent}% burn down remaining`);
      }
      ...
      if (weeklyUsage) {
        ...
        console.log(`Added weekly usage quota: ${pagePercent}% page value → ${burnDownPercent}% burn down remaining`);
      }
