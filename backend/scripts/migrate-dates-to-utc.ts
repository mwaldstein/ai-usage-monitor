...
async function migrateDatesToUTC() {
  ...
  try {
    ...
    for (const service of services) {
      ...
      await db.run(
        'UPDATE services SET created_at = ?, updated_at = ? WHERE id = ?',
        [newCreatedAt, newUpdatedAt, service.id]
      );
    }
    ...
    for (const quota of quotas) {
      ...
      await db.run(
        'UPDATE quotas SET reset_at = ?, created_at = ?, updated_at = ? WHERE id = ?',
        [newResetAt, newCreatedAt, newUpdatedAt, quota.id]
      );
    }
    ...
    for (const record of history) {
      ...
      await db.run(
        'UPDATE usage_history SET timestamp = ? WHERE id = ?',
        [newTimestamp, record.id]
      );
    }
