...
export function UsageDock({ statuses, windowMinutes = 60 }: { statuses: ServiceStatus[]; windowMinutes?: number }) {
  ...
  const trends: TrendItem[] = useMemo(() => {
    ...
    for (const [key, list] of byKey) {
      ...
      const serviceName = serviceNameById.get(quota.serviceId) || list[0]?.service_name || quota.serviceId;
    ...
    <div className="mt-4 glass rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-zinc-500 font-mono">{windowMinutes >= 60 ? `${Math.round(windowMinutes / 60)}h` : `${windowMinutes}m`}</span>
        </div>
        <div className="flex items-center gap-2">
          {trends.length > 0 && (
