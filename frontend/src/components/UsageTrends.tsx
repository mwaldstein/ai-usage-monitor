...
function Sparkline({ values, width, height, stroke }: { values: number[]; width: number; height: number; stroke: string }) {
  const padding = 2;
...
function pickDisplaySeries(quota: UsageQuota, usedSeries: number[]): { series: number[]; last: number; delta: number; label: 'used' | 'remaining' } {
...
export function UsageTrends({ statuses, windowMinutes = 60 }: { statuses: ServiceStatus[]; windowMinutes?: number }) {
  ...
  const rows: TrendRow[] = useMemo(() => {
    ...
    for (const [key, list] of byKey) {
      ...
      const serviceName = serviceNameById.get(serviceId) || list[list.length - 1]?.service_name || serviceId;
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
      ...
      {!loading && rows.length === 0 && (
        <p className="text-sm text-gray-500">No quotas changed in the last {windowMinutes} minutes.</p>
      )}
      ...
      {rows.length > 0 && (
        <div className="space-y-3">
          {rows.map((row) => {
            ...
            const isBurnDown = quotaType === 'usage' || quotaType === 'credits';
            const stroke = isBurnDown
              ? (row.delta < 0 ? '#ef4444' : '#16a34a')
              : '#2563eb';
            ...
            return (
              <div key={row.key} className="flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <div className="text-sm font-medium text-gray-900 truncate">{row.serviceName}</div>
                  <div className="text-xs text-gray-500 truncate">{metricLabel}</div>
                </div>

                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <div className={`text-sm font-semibold ${stroke === '#ef4444' ? 'text-red-600' : stroke === '#16a34a' ? 'text-green-700' : 'text-blue-700'}`}>
                      {deltaText}
                    </div>
